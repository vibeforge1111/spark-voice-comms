...
def _validate_env_file_path(env_file_path: str) -> Path:
    """Validate env_file_path rejects traversal attempts and resolves cleanly."""
    if ".." in Path(env_file_path).parts:
        raise ValueError(f"env_file_path must not contain '..' segments: '{env_file_path}'.")
